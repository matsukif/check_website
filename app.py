...
#    "https://news.yahoo.co.jp/articles/bcd35011488c2d67cc7785f1bb8e40a001cd9ffb",
#]

url = st.sidebar.text_input("URLを入力してね")
urls = [url]
...
if url:
#  with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
#    tmp_file.write(web_data.getvalue())
#    tmp_file_path = tmp_file.name
...
#  loader = PyPDFLoader(file_path=tmp_file_path)
  ...
  if 'generated' not in st.session_state: #['generated']にてモデルから生成された回答を保存
    st.session_state['generated'] = ["このWebサイトについてなんでも聞いてね\n\n"]
  ...
  with container:
    with st.form(key='my_form', clear_on_submit=True):
      ...
      user_input = st.text_input("input:", placeholder="質問を入力してね", key='input')
